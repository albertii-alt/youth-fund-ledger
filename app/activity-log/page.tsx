...
export default function ActivityLogPage() {
  ...
  function ask(label: string, onConfirm: () => Promise<void>) { setConfirm({ label, onConfirm }); }
  ...
  async function hideOne(id: number) {
    await fetch(`/api/activity-log/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'hide' }) });
  }
  async function hideAll() { await fetch('/api/activity-log/hide-all', { method: 'POST' }); }
  async function restoreOne(id: number) {
    await fetch(`/api/activity-log/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'restore' }) });
  }
  async function deleteOne(id: number) { await fetch(`/api/activity-log/${id}`, { method: 'DELETE' }); }
  async function purgeAll() {
    const body: Record<string, string> = {};
    if (memberId) body.member_id = memberId;
    await fetch('/api/activity-log/purge', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
  }
    ...
    <main className="main">
      ...
      {loggedIn && (
        <div className="activity-log-tabs">
          <button className={tab === 'visible' ? 'activity-tab activity-tab-active' : 'activity-tab'} onClick={() => setTab('visible')}>Visible</button>
          <button className={tab === 'hidden' ? 'activity-tab activity-tab-active' : 'activity-tab'} onClick={() => setTab('hidden')}>Hidden</button>
        </div>
      )}
        ...
        <div className="modal-overlay" onClick={() => setConfirm(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              ...
              <button className="btn-danger" onClick={runConfirm} disabled={acting}>{acting ? 'Working…' : 'Confirm'}</button>
            </div>
          </div>
        </div>
      )}
